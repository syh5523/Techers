...
cUIObject::cUIObject()
	:m_isHidden(false), m_vPosition(0, 0, 0), m_pParent(NULL), m_stSize(0, 0)
	, m_vecChild(NULL), m_nTag(0)
{
...
void cUIObject::Update()
{
	...
	for each(auto c in m_vecChild) c->Update();
...
void cUIObject::Render(LPD3DXSPRITE pSprite)
{
	...
	for each(auto c in m_vecChild) c->Render(pSprite);
}
...
void cUIObject::Destroy()
{
	...
	for each(auto c in m_vecChild) c->Destroy();
